...
/**
 * Heuristic, dependency-free root detection. No Play Integrity, no network call — this app's
 * whole story is "nobody else is involved," and a live check-in to Google on every launch would
 * contradict that. Combines several independent signals and requires at least one *strong* one
 * before reporting root, so a single weak/coincidental match (e.g. test-keys on a legitimate
 * custom ROM) doesn't lock out a real user.
 *
 * Deliberately a plain object with zero Gradle dependencies and no DI annotations: this exact
 * file is meant to be copied byte-for-byte into every unpruuf Android app (relay-android today,
 * a future Case Vault client later) — those don't all use Hilt, and there's no shared Android
 * module between them.
 *
