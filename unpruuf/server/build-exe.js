#!/usr/bin/env node
// Builds a single, standalone unpruuf-relay.exe that needs no separate Node.js install to RUN
// (only to BUILD, here, once) — see EXE_BUILD.md for the full explanation and verification notes.
//
...
// src/sqliteNativeBinding.ts for how that gets loaded back at runtime (verified end-to-end,
// including a real SQLite write+read round-trip, before this script was written this way).
...
const OUT_DIR = path.join(ROOT, "build", "unpruuf-relay");
const EXE_NAME = "unpruuf-relay.exe";
...
function main() {
  if (process.platform !== "win32") {
    console.warn(
      "[build-exe] WARNING: not running on Windows. The resulting file will only run on this " +
        ...
        "unpruuf-relay.exe others can use without installing Node.js.\n"
    );
  }

  if (!fs.existsSync(path.join(ROOT, "node_modules", "typescript"))) {
    step("0/6 Dependencies not installed yet — running npm install first...");
    run("npm", ["install"], { shell: process.platform === "win32" });
  }

  step("1/6 Compiling TypeScript (npm run build)...");
  ...
  // here fails with EINVAL (confirmed against a real Windows run, not assumed — see
  // CHANGELOG.md). Not needed for postject/node further down since those spawn a real .exe.
  run("npm", ["run", "build"], { shell: process.platform === "win32" });

  step("2/6 Bundling into a single file (esbuild)...");
  ...
  // confirmed by hitting exactly that here; the API is the stable, documented way to call it.
  ...
  step("3/6 Preparing the Node SEA blob...");
  ...
  step("4/6 Copying the Node binary and injecting the app into it...");
  ...
  step("5/6 Copying the better-sqlite3 native addon (can't be embedded in the blob itself)...");
  ...
  step("6/6 Setting up the Tor folder (Tor itself downloads automatically on first run)...");
  fs.mkdirSync(path.join(OUT_DIR, "windows", "tor"), { recursive: true });
  ...
  console.log(`\n[build-exe] Done. Hand out the whole "${path.relative(ROOT, OUT_DIR)}" folder — ` +
    `keep dist/, native/, and windows/ together, don't move the .exe out on its own.\n` +
    `Double-clicking dist\\${EXE_NAME} is now the entire setup for whoever receives it: ` +
    `no Node.js, no separate Tor download step, nothing else to install.`);
}
